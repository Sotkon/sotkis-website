export const contactTranslations = {
  pt: {
    title: "Fale connosco!",
    subtitle: "Se precisa de informações sobre os nossos produtos e serviços",
    success: "Obrigado! Entraremos em contacto em breve.",
    error: "Algo correu mal. Por favor, tente novamente.",
    privacy: {
      policy: "Ao introduzir os meus dados eu aceito a [Política de Privacidade e Cookies da Sotkon](https://sotkon.com/pt/privacy-policy/)",
      newsletter: "Ao introduzir os meus dados, autorizo a receção da newsletter da Sotkon"
    },
    form: {
      contactType: {
        placeholder: "Tipo de contacto *",
        newsletter: "Newsletter",
        distributor: "Tornar-me Distribuidor",
        information: "Receber mais informação",
      },
      name: { label: "Nome *", placeholder: "O seu nome" },
      email: { label: "Correio Eletrónico *", placeholder: "exemplo@email.com" },
      phone: { label: "Telefone", placeholder: "+351 900 000 000" },
      companyName: { label: "Nome da Empresa", placeholder: "Nome da Empresa" },
      country: { label: "País", placeholder: "País" },
      comments: { label: "Comentários", placeholder: "Comentários / Comments / Commentaires" },
      submit: "Enviar Mensagem",
      submitting: "A enviar...",
    }
  },
  en: {
    title: "Contact us!",
    subtitle: "If you need information about our products and services",
    success: "Thank you! We will be in touch soon.",
    error: "Something went wrong. Please try again.",
    privacy: {
      policy: "By entering my data, I accept Sotkon's [Privacy and Cookies Policy](https://sotkon.com/en/privacy-cookie-policy/)",
      newsletter: "By entering my data, I authorize the receipt of Sotkon's newsletter"
    },
    form: {
      contactType: {
        placeholder: "Select a Service *",
        newsletter: "Newsletter",
        distributor: "Become a Distributor",
        information: "Receive more information",
      },
      name: { label: "Name *", placeholder: "Name" },
      email: { label: "Email *", placeholder: "Email" },
      phone: { label: "Phone", placeholder: "+351 900 000 000" },
      companyName: { label: "Company", placeholder: "Company" },
      country: { label: "Country", placeholder: "Country" },
      comments: { label: "Message", placeholder: "Message" },
      submit: "Send Message",
      submitting: "Sending...",
    }
  },
  es: {
    title: "Hable con nosotros",
    subtitle: "Si necesita información sobre nuestros productos y servicios.",
    success: "¡Gracias! Nos pondremos en contacto pronto.",
    error: "Algo salió mal. Por favor, inténtelo de nuevo.",
    privacy: {
      policy: "Al introducir mis datos acepto la [Política de Privacidad y Cookies de Sotkon](https://sotkon.com/en/privacy-cookie-policy/)",
      newsletter: "Al introducir mis datos, autorizo la recepción del boletín de Sotkon"
    },
    form: {
      contactType: {
        placeholder: "Tipo de contacto *",
        newsletter: "Newsletter",
        distributor: "Convertirse en Distribuidor",
        information: "Recibir más información",
      },
      name: { label: "Nombre *", placeholder: "Su nombre" },
      email: { label: "Correo Electrónico *", placeholder: "ejemplo@email.com" },
      phone: { label: "Teléfono", placeholder: "+351 900 000 000" },
      companyName: { label: "Nombre de la Empresa", placeholder: "Nombre de la Empresa" },
      country: { label: "País", placeholder: "País" },
      comments: { label: "Comentarios", placeholder: "Comentarios / Comments / Commentaires" },
      submit: "Enviar Mensaje",
      submitting: "Enviando...",
    }
  },
  fr: {
    title: "Parlez-nous",
    subtitle: "Si vous avez besoin d'informations sur nos produits et services.",
    success: "Merci ! Nous vous contacterons bientôt.",
    error: "Quelque chose s'est mal passé. Veuillez réessayer.",
    privacy: {
      policy: "En saisissant mes données, j'accepte la [Politique de Confidentialité et Cookies de Sotkon](https://sotkon.com/en/privacy-cookie-policy/)",
      newsletter: "En saisissant mes données, j'autorise la réception de la newsletter de Sotkon"
    },
    form: {
      contactType: {
        placeholder: "Type de contact *",
        newsletter: "Newsletter",
        distributor: "Devenir Distributeur",
        information: "Recevoir plus d'informations",
      },
      name: { label: "Nom *", placeholder: "Votre nom" },
      email: { label: "Email *", placeholder: "exemple@email.com" },
      phone: { label: "Téléphone", placeholder: "+351 900 000 000" },
      companyName: { label: "Nom de l'Entreprise", placeholder: "Nom de l'Entreprise" },
      country: { label: "Pays", placeholder: "Pays" },
      comments: { label: "Commentaires", placeholder: "Commentários / Comments / Commentaires" },
      submit: "Envoyer le Message",
      submitting: "Envoi en cours...",
    }
  },
  gr: {
    title: "Talk to us",
    subtitle: "If you need information about our products and services.",
    success: "Thank you! We will be in touch soon.",
    error: "Something went wrong. Please try again.",
    privacy: {
      policy: "By entering my data I accept Sotkon's [Privacy and Cookie Policy](https://sotkon.com/en/privacy-cookie-policy/)",
      newsletter: "By entering my data, I authorize the receipt of Sotkon's newsletter"
    },
    form: {
      contactType: {
        placeholder: "Type of contact *",
        newsletter: "Newsletter",
        distributor: "Become a Distributor",
        information: "Receive more information",
      },
      name: { label: "Name *", placeholder: "Your name" },
      email: { label: "Email *", placeholder: "example@email.com" },
      phone: { label: "Phone", placeholder: "+351 900 000 000" },
      companyName: { label: "Company Name", placeholder: "Company Name" },
      country: { label: "Country", placeholder: "Country" },
      comments: { label: "Comments", placeholder: "Comments / Commentários / Commentaires" },
      submit: "Send Message",
      submitting: "Sending...",
    }
  },
  cr: {
    title: "Talk to us",
    subtitle: "If you need information about our products and services.",
    success: "Thank you! We will be in touch soon.",
    error: "Something went wrong. Please try again.",
    privacy: {
      policy: "By entering my data I accept Sotkon's [Privacy and Cookie Policy](https://sotkon.com/en/privacy-cookie-policy/)",
      newsletter: "By entering my data, I authorize the receipt of Sotkon's newsletter"
    },
    form: {
      contactType: {
        placeholder: "Type of contact *",
        newsletter: "Newsletter",
        distributor: "Become a Distributor",
        information: "Receive more information",
      },
      name: { label: "Name *", placeholder: "Your name" },
      email: { label: "Email *", placeholder: "example@email.com" },
      phone: { label: "Phone", placeholder: "+351 900 000 000" },
      companyName: { label: "Company Name", placeholder: "Company Name" },
      country: { label: "Country", placeholder: "Country" },
      comments: { label: "Comments", placeholder: "Comments / Commentários / Commentaires" },
      submit: "Send Message",
      submitting: "Sending...",
    }
  }
};
